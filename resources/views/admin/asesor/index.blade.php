...
@section('content')
    <div class="flex justify-between items-start">
        ...
        <a href="{{ route('admin.asesor.create') }}"
            ...
            <i class="fas fa-plus mr-1.5"></i> Tambah Asesor
        </a>
    </div>
                            ...
                            <td>
                                ...
                                <div id="dropdown-{{ $loop->iteration }}"
                                    ...
                                    <ul class="py-2 text-sm text-gray-700" aria-labelledby="dropdownDefaultButton">
                                        <li>
                                            <a href="javascript:void(0)" data-id="{{ $asesor->id }}"
                                                data-modal-target="detail-modal" data-modal-toggle="detail-modal"
                                                class="btn-show-detail block px-4 py-2 hover:bg-gray-100">
                                                Lihat
                                                Detail
                                            </a>
                                        </li>
...
@section('script')
    <script>
        ...
        function showDetail() {
            ...
            $.ajax({
                url: "{{ route('admin.asesor.show', ':id') }}".replace(':id', id),
                type: "GET",
                beforeSend: function() {
        ...
        function deleteEvent() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ route('admin.asesor.destroy', ':id') }}`.replace(':id', id),
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}",
                        },
