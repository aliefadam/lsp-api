...
@section('content')
    <form action="{{ route('admin.event.update', $event->id) }}" method="POST" class="" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="bg-white lg:w-1/2 w-full rounded-md shadow-md p-5 h-fit">
                ...
                <div class="mb-5">
                    ...
                    <select id="place" name="place"
                        ...
                        @foreach ($tuks as $tuk)
                            <option @selected($tuk->name . ' - ' . $tuk->address == $event->place) value="{{ $tuk->name }} - {{ $tuk->address }}">
                ...
                <div class="mb-5">
                    ...
                    <textarea id="desc" name="desc" rows="4"
                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-orange-500 focus:border-orange-500">{{ $event->desc }}</textarea>
                </div>
                <div class="mb-5">
                    <label for="desc" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    ...
                    @php
                        $selectedSchemes = $event->schedules->pluck('scheme_id')->toArray();
                    @endphp
